...
if __name__ == "__main__":
    url_inicial = input("\n🔎 Informe a URL inicial: ")
    while True:
        entrada = input("\nInforme a quantidade de clusters de pesquisa (0 a 100): ")
        try:
            num_clusters = int(entrada)
            if 0 <= num_clusters <= 100:
                break  # entrada válida, sai do loop
            else:
                print("Valor fora do intervalo permitido. Digite um número entre 0 e 100.")
        except ValueError:
            print("Entrada inválida. Por favor, digite apenas números inteiros.")
    todos_os_links = crawler(url_inicial, num_clusters)
    print(f"\n✅ Total de URLs encontrados: {len(todos_os_links)}\n")
